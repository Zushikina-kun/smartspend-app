...
class _PalawaganScreenState extends State<PalawaganScreen> {
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              Row(children: [
                const Text("Current round: ", style: TextStyle(fontSize: 13)),
                IconButton(
                    icon: const Icon(Icons.remove_circle_outline),
                    onPressed: currentRound > 1 ? () => setS(() => currentRound--) : null),
                Text('$currentRound', style: const TextStyle(fontWeight: FontWeight.bold)),
                IconButton(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (existing != null) {
                await DBService.updatePalawagan({...data, 'id': existing['id']});
              } else {
  ...
  Future<void> _logContribution(Map<String, dynamic> g) async {
    ...
    // Advance round
    await DBService.updatePalawagan({...g, 'current_round': (g['current_round'] as int) + 1});
          ...
          : _groups.isEmpty
              ? Center(
                  child: Padding(
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text("Track your rotating savings group — members, payout order, and contributions.",
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final winner = currentRound <= totalRounds && members.isNotEmpty
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(children: [
                            ...
                            PopupMenuButton<String>(
                              onSelected: (v) {
                                if (v == 'edit') _showAddGroupDialog(existing: g);
                                if (v == 'delete') DBService.deletePalawagan(g['id'] as int).then((_) => _load());
                              },
                              itemBuilder: (_) => [
                          ...
                          const SizedBox(height: 8),
                          Row(children: [
                            _statChip("₱${amt.toStringAsFixed(0)}/round", Icons.payments_outlined, cs.primary),
                            const SizedBox(width: 8),
                            _statChip("Pot: ₱${potSize.toStringAsFixed(0)}", Icons.savings_outlined, Colors.green),
                            const SizedBox(width: 8),
                            _statChip("Round $currentRound/${members.isEmpty ? '?' : totalRounds}", Icons.loop, Colors.orange),
                          ]),
                          if (winner != null) ...[
                            const SizedBox(height: 8),
                            Container(
                              ...
                              children: members.asMap().entries.map((e) {
                                ...
                                return Container(
                                  ...
                                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                                    if (isPast) const Icon(Icons.check, size: 10, color: Colors.green),
                                    if (isCurrent) const Text("🎉", style: TextStyle(fontSize: 10)),
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              label: Text("Log My Contribution (₱${amt.toStringAsFixed(0)})"),
