...
|---------|------|-------------|
| 2.8.0 | June 13, 2026 | 29 AI actions (+split_expense auto-debt, +task-routing fast/smart), Financial Milestones card (Analytics), Compact Mode via ThemeService.visualDensity, split_with shown in expense tiles, score_service.dart bug fixed, SMARTSPEND_MASTER_ROADMAP.md, all coding tasks complete. Multi-model: Gemini 2.5 Flash-Lite → Flash → Groq 70B → 8B → Cerebras, typing indicator, long-press menu, BIR breakdown, PFKB, Financial Glossary (23 terms), wallet-first setup, Analytics toggle |
